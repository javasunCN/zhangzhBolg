...
### 模式动机
```text
    1、在工厂方法模式中具体工厂负责生产具体的产品，每一个具体工厂对应一种具体产品，工厂方法也具有唯一性，
...
```text
    ...
    工厂模式的退化
        1、当抽象工厂模式中每一个具体工厂类只创建一个产品对象，也就是只存在一个产品等级结构时，
            抽象工厂模式退化成工厂方法模式；当工厂方法模式中抽象工厂与具体工厂合并，提供一个统一的工厂来创建产品对象，
            并将创建对象的工厂方法设计为静态方法时，工厂方法模式退化成简单工厂模式。
```
...
```text
    ...
    4、抽象工厂模式的主要优点是隔离了具体类的生成，使得客户并不需要知道什么被创建，而且每次可以通过具体工厂类创建一个产品族中的多个对象，
        ...
        对“开闭原则”的支持呈现倾斜性。
    5、抽象工厂模式适用情况包括：一个系统不应当依赖于产品类实例如何被创建、组合和表达的细节；系统中有多于一个的产品族，
...
```java
     ...
     * 具体产品角色(产品等级) - 长虹视机
     */
    public class ConcreteProductChanghong implements ProductA {
        @Override
        public void produce() {
            System.out.println("长虹视机");
        }
    }
    /**
     * 具体产品角色(产品等级) - 海尔电视机
     */
    public class ConcreteProductHarer implements ProductA {
        @Override
        public void produce() {
     ...
     * 具体产品角色(产品等级) - 格力空调
     */
    public class ConcreteProductGeli implements ProductB {
        @Override
        public void produce() {
     ...
     * 具体产品角色(产品等级) - 海尔空调
     */
    public class ConcreteProductHarer implements ProductB {
        @Override
        public void produce() {
    ...
    public class ChanghongFactory implements AbstractFactory {
        ...
        @Override
        public ProductA productTV() {
            return new ConcreteProductChanghong();
        }
    
        @Override
        public ProductB productAir() {
            return null;
        }
    }
    /**
    ...
    public class GeliFactory implements AbstractFactory {
        ...
        @Override
        public ProductB productAir() {
            return new ConcreteProductGeli();
        }
    }
    /**
     * 具体工厂角色 - 海尔工厂
    ...
    public class HaierFactory implements AbstractFactory {
        ...
        @Override
        public ProductA productTV() {
            return new ConcreteProductHarer();
        }
    
        @Override
        public ProductB productAir() {
            return new com.design.cn.创建型.抽象工厂模式.B.ConcreteProductHarer();
        }
    }
    ...
    public class Main {
        ...
        public static void main(String[] args) {
            ...
            // 长虹工厂
            AbstractFactory changHongFactory = new ChanghongFactory();
            changHongFactory.productTV().produce();
    
            // 海尔工厂
            AbstractFactory haierFactory = new HaierFactory();
            haierFactory.productTV().produce();
            haierFactory.productAir().produce();
    
            // 格力工厂
            AbstractFactory geliFactory = new GeliFactory();
            geliFactory.productAir().produce();
